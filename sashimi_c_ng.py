...
class halo_model(cosmology):
    ...
    def conc200(self,M200,z): 
        """ Correa et al. (2015-III), also summarised in Appendix B"""
    ...
    def Mvir_from_M200(self, M200, z):
        ...
        # Mvir2  = 4*np.pi*rvir**3*Dc*self.rhocrit0*gz/3. # this is same as above due to the definition of rvir
        return Mvir
    ...
    def Mzi(self, M0, z):
        """Correa et al. (2015-I), also summarised in Appendix A"""
...
# Adding the non-Gaussian factor R    
    def S3_fit(self, f_NL, M):
        """
    ...
    def R_MVJ(self, M, z, f_NL): 
        ...
        R_MVJ = exp_term * ((1/6) * (delta_c / sqrt_term) * dS3dlnsigma + sqrt_term)
        return R_MVJ


    def R_LMSV(self, M, z, f_NL):
        """
        The non-Gaussian correction factor to the halo mass function from LoVerde et al. (2008)
        """
        beta = 0.4
        delta_c = self.deltac_func(z)
        S3 = self.S3_fit(f_NL, M)
        ...
        R_LMSV = 1 + (mass_var / (6 * delta_c)) * (S3 * bracket_quartic + dS3dlnsigma * bracket_quadratic)
        return R_LMSV
    ...
    def Ffunc_Yang_gen(self, delc1, delc2, s1, s2, M=None, z=None, f_NL=0., NG_model="Gaussian"):
        """
        Generalised form of Eq. (14) of Yang et al. (2011) including the non-Gaussian factor R.
        NG_model : {"Gaussian", "MVJ", "LMSV"}
        Ffunc_Yang_gen(delcM,delca,sM,sa, ma, zacc_2d, f_NL, NG_model)
        """
        Ffunc_Yang = 1./np.sqrt(2.*np.pi)*(delc2-delc1)/(s2-s1)**1.5 \
            *np.exp(-(delc2-delc1)**2/(2.*(s2-s1)))

        if NG_model == "Gaussian":
            return Ffunc_Yang
        elif NG_model == "MVJ":
    ...
    def Na_calc(self, ma, zacc, Mhost, f_NL=0., NG_model="Gaussian", z0=0., N_herm=200, Nrand=1000, Na_model=3):
        ...
        if Na_model==3:
            ...
            normB    = special.gamma(0.5)*special.gammainc(0.5,xmax)/np.sqrt(np.pi) # numerical?
            # those reside in the exponential part of Eq. (14) 
            Phi      = self.Ffunc_Yang_gen(delcM,delca,sM,sa,M=ma,z=zacc_2d,
                                           f_NL=f_NL,NG_model=NG_model)/normB*np.heaviside(mmax-ma,0)
        elif Na_model==1:
            delca    = self.deltac_func(zacc_2d)
            sM       = self.s_func(M200)
            sa       = self.s_func(ma)
    ...
    def subhalo_properties_calc(self, M0, f_NL=0., NG_model="Gaussian", redshift=0.0, dz=0.1, zmax=7.0, 
                                N_ma=500, sigmalogc=0.128, N_herm=5, logmamin=-6, logmamax=None, N_hermNa=200, 
                                Na_model=3, ct_th=0.77, profile_change=True, M0_at_redshift=False):
        """
