...
class FamilyManifestTests(unittest.TestCase):
    def setUp(self):
        ...
        for name in ("check_compatibility.py", "check_artifact_provenance.py"):
            shutil.copyfile(
                Path(FAMILY_SOURCE) / "scripts" / name, self.family / "scripts" / name
            )
        self.git("init", "--quiet")
        ...
        for index, name in enumerate(MODULE.PACKAGES, start=1):
            ...
            self.git(
                "update-index", "--add", "--cacheinfo", f"160000,{revision},{name}"
            )
    ...
    def git(self, *args):
        return subprocess.check_output(
            ["git", "-C", str(self.family), *args], text=True
        ).strip()

    def prepare(self, **kwargs):
        return MODULE.prepare(
            self.family, self.output, "sashimi-f", self.candidate, **kwargs
        )
    ...
    def test_coordinated_core_candidate_is_recorded_without_changing_other_siblings(
        self,
    ):
        revision = "b" * 40
        _, effective = self.prepare(overrides={"itamae": revision})
        self.assertEqual(effective["itamae"]["ref"], revision)
        self.assertEqual(effective["sashimi-c"], self.base["sashimi-c"])
        record = json.loads(self.output.with_suffix(".json").read_text())
        self.assertEqual(
            record["overrides"], {"sashimi-f": self.candidate, "itamae": revision}
        )

    def test_coordinated_overrides_are_immutable_and_unambiguous(self):
        for overrides in (
            {"unknown": "b" * 40},
            {"itamae": "main"},
            {"sashimi-f": "b" * 40},
            ...
            with self.subTest(overrides=overrides), self.assertRaises(ValueError):
                self.prepare(overrides=overrides)
        with self.assertRaisesRegex(ValueError, "cannot apply overrides"):
            self.prepare(mode="promoted", overrides={"itamae": "b" * 40})
    ...
    def test_c_candidate_keeps_f_and_other_sibling_revisions(self):
        _, effective = MODULE.prepare(
            self.family, self.output, "sashimi-c", self.candidate
        )
    ...
    def test_refuses_canonical_output_path(self):
        with self.assertRaisesRegex(ValueError, "overwrite"):
            MODULE.prepare(
                self.family,
                self.family / "compatibility.toml",
                "sashimi-f",
                self.candidate,
            )
    ...
    def test_wrong_candidate_wheel_provenance_is_rejected(self):
        ...
        for revision, expected_code in (("5" * 40, 1), (self.candidate, 0)):
            with zipfile.ZipFile(artifact, "w") as archive:
                archive.writestr(
                    "_sashimi_f_build_provenance.py",
